...
class Izvestaj extends React.Component {    
    ...
    createData(rb, id, vlasnik, fiskalnaGodina, datumDonosenjaOdluke, datumStanjaNaDan, iznos) {
        return { rb, id, vlasnik, fiskalnaGodina, datumDonosenjaOdluke, datumStanjaNaDan, iznos };
    }

    ucitajSveIzvestaje = () => {
        fetch(`${api_path}/izvestaj`)
    ...
    ucitajIzvestaj = (id) => {
        ...
        .then(response => {
            ...
            this.setState({
                ...
                datumDonosenjaOdluke : (new Date(izvestaj.datumDonosenjaOdluke)).toLocaleDateString(),
                datumStanjaNaDan : (new Date(izvestaj.datumStanjaNaDan)).toLocaleDateString(),
    ...
    pripremiIzvestajeZaTabPrikaz = () => {
        let svi = []
        this.state.sviIzvestaji.forEach((e, index) => svi.push(this.createData(
            index + 1,
            e._id,
            e.vlasnik,
            e.fiskalnaGodina,
    ...
    izabraniIzvestaj = (izvestajIzTabele) => {
        this.setState({
            ...
            izabrani_datumDonosenjaOdluke : new Date(izvestajIzTabele.datumDonosenjaOdluke),
    ...
    sacuvajIzvestaj = (izvestaj) => {
        ...
        if (izvestaj.check) {
            ...
            if (izvestaj.id === null || izvestaj.id === undefined || izvestaj.id.trim() === "") {
                this.openModal(`Iz tabele izaberite izveštaj koji želite da ažurirate, 
                    ili označite čuvanje novog izveštaja!`, "error");
            }            
            // Patch request          
            path = `${api_path}/izvestaj/${izvestaj.id}`
            requestOptions = {
                method: 'PATCH',
                headers: myHeaders,
                body: raw,
    ...
    obrisiIzvestaj = (izvestaj) => {
        ...
        let path = `${api_path}/izvestaj/${izvestaj.id}`
        ...
        return(
            <div className='izvestajPage'>
                ...
                }  
                {
                    ...
                    <Snackbar 
                        open={this.openModal} anchorOrigin={{ vertical: 'top', horizontal: 'center' }}
                        autoHideDuration={5000} onClose={this.closeModal}
                    >
                        <Alert 
                            severity={this.state.modalTip}
