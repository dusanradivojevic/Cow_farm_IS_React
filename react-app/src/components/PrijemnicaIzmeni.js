...
function PrijemnicaIzmeni() {
  ...
  const [otpremnica, setOtpremnica] = useState({ stavke: [] });
  ...
  useEffect(() => {
    if (prijemnica.dobavljac === undefined) return;

    // Punjenje forme
    setDobavljac(prijemnica.dobavljac);
    setSkladiste(prijemnica.skladiste);
    setBrojOtpremnice(prijemnica.brojOtpremnice);
    setSifraZap(prijemnica.zaposleni.sifra);
    setImeZap(prijemnica.zaposleni.ime);
    setPrezimeZap(prijemnica.zaposleni.prezime);
  }, [prijemnica]);

  useEffect(() => {
    if (dobavljac.naziv !== "") vratiOtpremnicu();
  }, [dobavljac]);
  ...
  const vratiOtpremnicu = () => {
    if (brojOtpremnice === "") {
      openModal("Unesite broj otpremnice!", "error");
      return;
    }
  ...
  const vratiPrijemnicu = () => {
    if (brojPrijemnice === "") {
      openModal("Unesite broj prijemnice!", "error");
      return;
    }
    ...
    fetch(`/api/prijemnica/${brojPrijemnice}`)
      ...
      .then((result) => {
        if (result.length !== 0) {
          setPrijemnica(result[0]);

          result[0].stavke.length <= 5
            ? setRowsPerPage2(result[0].stavke.length)
            : setRowsPerPage2(5);
        } else {
          openModal("Ne postoji prijemnica sa tim brojem!", "error");
          setBrojOtpremnice("");
          setOtpremnica({ stavke: [] });
          setPrijemnica({ brojPrijemnice: "", stavke: [] });
          setSkladiste("");
  ...
  const emptyRows =
    rowsPerPage -
    Math.min(rowsPerPage, otpremnica.stavke.length - page * rowsPerPage);
  ...
  const prikaziStavku = () => {
    const stavka = otpremnica.stavke.filter(
      (row) => row.redniBroj === selected
    );
  ...
  const potvrdiIzmene = () => {
    ...
    const postoji = prijemnica.stavke.filter(
      (row) => row.redniBroj === novaStavka.redniBroj
    );
    if (postoji.length > 0) {
  ...
  const obrisiStavku = () => {
    const stavka = prijemnica.stavke.filter(
      (row) => row.redniBroj === selected2
    );

    if (stavka[0] !== undefined) {
      let prm = prijemnica;
      prm.stavke = prijemnica.stavke.filter(
        (row) => row.redniBroj !== selected2
      );
      setPrijemnica(prm);
      setRowsPerPage2(prm.stavke.length);
    } else {
      openModal("Morate izabrati stavku!", openModal);
      return;
    }
  };
  /// END TABELA
  ...
  const validacijaPrijemnice = () => {
    if (brojOtpremnice === "")
      return { status: "err", message: "Izaberite prijemnicu!" };
    ...
    if (isNaN(new Date(datum).getDay()))
      return { status: "err", msg: "Neispravan datum!" };

    if (otpremnica.stavke.length !== prijemnica.stavke.length)
      return {
        status: "err",
        message: "Morate uneti sve stavke iz otpremnice!",
      };

    if (imeZap === "") return { status: "err", message: "Unesite zaposlenog!" };
    ...
    <div className="prijemnicaPage">
      <div className="prviRed">
        <div className="pronadjiOtpremnicu">
          <label>Prijemnica</label>
          <br />
          <TextField
            id="outlined-basic"
            label="Broj prijemnice"
            variant="outlined"
            margin="normal"
            type="number"
            className="sifraOtpremniceInput"
            name="sifraPrijemnice"
            onChange={(e) => setBrojPrijemnice(e.target.value)}
            value={brojPrijemnice}
          />
          <br />
          <br />
          <Button
            variant="contained"
            className="pronadjiOtpBtn"
            margin="none"
            onClick={vratiPrijemnicu}
          >
            Pronađi prijemnicu
          </Button>
          <p></p>
        ...
        <div className="sredina">
          <TextField
            ...
            disabled
            name="brojPrijemnice"
            value={prijemnica.brojPrijemnice}
          />
          <br />
          <TextField
            id="outlined-basic"
            ...
            margin="dense"
            type="text"
            onChange={(e) => setSkladiste(e.target.value)}
            name="skladiste"
            value={skladiste}
          />
          <br />
        ...
        <div className="dobavljac">
          ...
          <TextField
            id="outlined-basic"
            label="Šifra dobavljača"
            variant="outlined"
            margin="dense"
            type="text"
            disabled
            name="sifraDobavljaca"
            value={dobavljac.sifra}
          />
          <br />
          <TextField
            id="outlined-basic"
      ...
      <div className="prijemnicaTabela">
        ...
        <br />
        <label>Obriši stavku prijemnice</label>
      </div>
      {/* Zaposleni */}
      <div className="petiRed">
        <div className="pronadjiZaposlenog">
          ...
          <div className="pretragaZap">
            <TextField
              id="outlined-basic"
              label="Šifra zaposlenog"
              variant="outlined"
              margin="normal"
              type="number"
              className="sifraZaposlenogInput"
              onChange={(e) => setSifraZap(e.target.value)}
              name="sifraOtpremnice"
              value={sifraZap}
            />
            <Button
              variant="contained"
              className="pronadjiZapBtn"
              margin="none"
        ...
        <TextField
          id="outlined-basic"
          label="Prijemnicu kreirao"
          variant="outlined"
          margin="normal"
          type="text"
          className="imeZaposlenogInput"
          disabled
          name="imeZaposlenog"
          value={imeZap + " " + prezimeZap}
        />
      </div>
      <Button
        variant="contained"
        className="sacuvajPrmBtn"
        margin="none"
        onClick={sacuvajPrijemnicu}
        color="secondary"
      >
        Ažuriraj prijemnicu
      </Button>
      ...
      {open ? (
        <Snackbar
          ...
          onClose={closeModal}
        >
          <Alert severity={tipModala} onClose={closeModal}>
            {tekstModala}
          </Alert>
        </Snackbar>
      ) : (
...
export default PrijemnicaIzmeni;
