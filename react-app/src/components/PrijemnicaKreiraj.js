...
function PrijemnicaKreiraj() {
  const [otpremnica, setOtpremnica] = useState({
    brojOtpremnice: "",
    stavke: [],
  });
  ...
  const vratiPrijemnicu = (broj) => {
    ...
    fetch(`/api/prijemnica/${broj}`)
      .then((response) => response.json())
      .then((result) => {
        if (result.length > 0) {
          return {
            status: "err",
            message: "Prijemnica sa tim brojem već postoji!",
          };
        }
      })
  ...
  const sacuvajPrijemnicu = () => {
    ...
    fetch(`/api/prijemnica`, requestOptions)
      .then((response) => response.json())
      ...
      <div className="prviRed">
        ...
        <div className="sredina">
          ...
          <TextField
            ...
            disabled
            name="brojOtpremnice"
            value={otpremnica.brojOtpremnice}
          />
          <br />
          <TextField
            margin="dense"
